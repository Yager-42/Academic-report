...
semophore=asyncio.Semaphore(50)#同步信号量，用来控制并发度，因为清华爬的快会封，所以改成同步爬取


def get_page_num(headers):
    base_url='https://www.math.pku.edu.cn/kxyj/xsbg/tlb/index.htm'
    response=requests.get(base_url,headers).content.decode('utf-8')
    soup=BeautifulSoup(response,'lxml')
    page_num=int(soup.find_all('div',class_='pages')[0].findAll('a')[-3].get_text())
    return page_num
...
def make_url(page_num):
    urls=[]
    urls.append('https://www.math.pku.edu.cn/kxyj/xsbg/tlb/index.htm')
    for num in range(1,page_num):
        urls.append('https://www.math.pku.edu.cn/kxyj/xsbg/tlb/index{}.htm'.format(num))
    return urls
...
def parse_html_for_urls(main_htmls):
    ...
    for html in main_htmls:
        ...
        main=soup.findAll('ul',class_='scholarsList')[0]
        lis=main.findAll('li')
        for li in lis:
            urls.append('http://www.math.pku.edu.cn/kxyj/xsbg/tlb/'+str(li.findAll('a')[0]['href']))
    return urls
...
def parse_html(htmls,check_list,urls):
    ...
    for index in range(len(htmls)):
        ...
        info_dic['题目']=soup.findAll('div',class_='subArticle_title')[0].get_text()
        info_dic['报告人']=findname(main_text)
        info_dic['通知时间']=info_dic['报告时间']=findtime(main_text)
