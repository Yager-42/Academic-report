...
def get_page_num(headers):
    base_url='http://iiis.tsinghua.edu.cn/list-265-1.html'
    response=requests.get(base_url,headers).text
    page_num=int(re.findall(r'<a href="/list-265-(\d+).html">末页</a>',response)[0])
    return page_num
...
def make_url(page_num):
    ...
    for num in range(1,page_num+1):
        urls.append('https://iiis.tsinghua.edu.cn/list-265-{}.html'.format(num))
    return urls
...
def parse_html_for_urls(main_htmls):
    ...
    for html in main_htmls:
        ...
        all_td=tbody.findAll('div',class_='td1')
        for td in all_td:
            urls.append('https://iiis.tsinghua.edu.cn'+str(td.findAll('a')[0]['href']))
    return urls
...
def parse_html(htmls,check_list,urls):
    ...
    for index in range(len(htmls)):
        info_dic={'链接':'','题目':'','通知时间':'','报告人':'','报告时间':'','地点':'','学校':'清华大学','mark':0}
        soup=BeautifulSoup(htmls[index],'lxml')
        main=soup.findAll('div',class_='news_info qh_desc2')[0]
        ...
        info_dic['题目']=soup.findAll('h2')[0].get_text()
